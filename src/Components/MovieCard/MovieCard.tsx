...
const MovieCard = ({image,title,id}:MovieCard) => {
  return (
    <div className="relative min-w-72 h-44 m-2 cursor-pointer hover:z-10 hover:scale-110 transform transition-transform duration-300 ease-out">
      {/* Movie Image */}
      <img
        src={IMAGE_URL + image}
        alt={title}
      ...
      <div className="absolute inset-0 bg-black bg-opacity-75 text-white flex flex-col justify-center items-center opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-lg ">
        <h3 className="text-xl absolute bottom-0 font-semibold mb-4 ">{title}</h3>
